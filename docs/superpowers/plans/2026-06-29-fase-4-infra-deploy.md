...
> **🔧 Atualização 2026-06-30 (filtros reais + achados de dado):** os filtros Grupo/Unidade/Especialidade usavam constantes mock (`AMBULATORIO A/B/C`…) que não batiam com a base → "Sem dados no recorte". Criado `GET /api/v1/dimensoes` (sql→provider→controller→router, 3 testes) que devolve os valores reais (7 grupos, **110 unidades** sem INATIVO, 705 especialidades); o front carrega via `useDimensoesStore` no mount do FilterBar. Verificado em produção (Playwright): selects populados, filtro por unidade real retorna 13.961 eventos, gargalos rankeia 10 unidades reais. **Achados de qualidade de dado (pro HC, não são bug):** (a) ~21 das 705 "especialidades" são na verdade nomes de enfermaria (`11º NORTE`…), preenchidos pelo AGHU em ~197k linhas de EXAME/CONSULTA; (b) unidades `- INATIVO` ficam fora do dropdown de filtro, mas ainda aparecem nos rankings/breakdowns (dado real) — ex.: `UTI RESPIRATORIA - INATIVO` é o topo do KPI-07. Decidir se exclui INATIVO também da analítica. **Caveat KPI-01 (635 dias):** mede prontuário→1º evento *presente na base*; prontuários vão de 2015-2026 e eventos anteriores à janela inflam o número — discutir âncora com o HC.
